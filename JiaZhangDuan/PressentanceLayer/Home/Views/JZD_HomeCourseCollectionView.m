...
- (NSArray *)courseArr {
    if (!_courseArr) {
        _courseArr = @[@{@"image":@"ic_yuwen", @"name":@"语文"}, @{@"image":@"ic_waijiao", @"name":@"外教"}, @{@"image":@"ic_quanke", @"name":@"全科"}, @{@"image":@"ic_onetoone", @"name":@"1对1"}];
    }
    return _courseArr;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dict = [self.courseArr objectAtIndex:indexPath.row];
    collectionViewCell.courseImage.image = [UIImage imageNamed:dict[@"image"]];
